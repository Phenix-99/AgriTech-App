trigger projectUpdateTrigger on Note__c (after insert, after update) {
    ...
    for(Note__c n:Trigger.New){
        ...
        Project__c proj =[SELECT Id,Etape_en_Cours__c,isUpdate__c,Status__c FROM Project__c WHERE Id=:n.Project__c LIMIT 1];
        system.debug('>>>>>Etape en cours'+n.Step_Rank__c);
        if(n.Step_Rank__c == proj.Etape_en_Cours__c && n.Description__c!=null && n.Description__c.length()>10){
