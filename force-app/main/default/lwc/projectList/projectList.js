...
import getFilteredProject from '@salesforce/apex/ProjectController.getFilteredProject';
...
export default class ProjectList extends NavigationMixin(LightningElement) {
    ...
    @wire(getFilteredProject, { country: '$filterValues.pays', region: '$filterValues.region', plant: '$filterValues.plant', status: '$filterValues.status' })
    getFarmer({data, error}) {
        if (data) {
            this.projectList = data.map(project => {
            const p = {
